...
const CameraUpload = () => {
  ...
  const [gender, setGender] = useState('F'); // 기본값을 'F'로 설정
  ...
  const handleConfirmAndUpload = async () => {
    ...
    if (!height || !weight || !gender) {
      toast({
        title: '신체 정보 필요',
        description: '키, 몸무게, 성별을 모두 입력해주세요.',
        status: 'warning',
        duration: 3000,
        isClosable: true,
      });
    ...
    try {
      ...
      formData.append('user_gender', gender); // 성별 추가
      ...
      const result = await response.json();
      console.log('Upload Result:', result); // 서버 응답을 확인하기 위한 로그

      if (!response.ok) {
        let errorMessage = result.detail || '업로드 실패';
        throw new Error(errorMessage);
      }

      // 서버 응답에 avatar_idx가 있는지 확인
      if (result && result.avatar_idx) {
        updateMeasurements({
          avatarIndex: result.avatar_idx,
          gender: gender,
        ...
        throw new Error('서버로부터 avatar_idx를 받지 못했습니다.');
      }
  ...
  return (
    <Box p={4} mt={12} mb={12} maxW="md" mx="auto">
      <Input
        type="file"
        accept="image/*"
        ref={fileInputRef}
        ...
        {(selectedImage || imagePreviewUrl) && (
          <VStack spacing={4} w="100%">
            <FormControl id="height" isRequired>
              ...
              <NumberInput
                value={height}
                onChange={(valueString) => setHeight(valueString)}
                min={50}
                max={250}
              >
