...
from sqlalchemy import Column, Integer, String, Text, ForeignKey, TIMESTAMP, Float, Enum
from sqlalchemy.orm import declarative_base, relationship
from datetime import datetime
import enum  # enum 모듈 추가
...
class User(Base):
    ...
    user_gender = Column(Enum(GenderEnum), nullable=False, comment='사용자 성별')  # 성별 필드 추가
    ...
    @classmethod
    def create(cls, user_id: str, user_name: str, hashed_password: str, user_email: str, user_gender: str) -> "User":
        return cls(
            user_id=user_id,
            user_pw=hashed_password,
            user_name=user_name,
            user_email=user_email,
            user_gender=GenderEnum(user_gender),  # 성별 추가
