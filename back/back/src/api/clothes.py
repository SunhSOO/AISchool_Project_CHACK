...
from utils.file_utils import validate_image_file, validate_3d_file, generate_unique_filename, save_upload_file, delete_file
...
os.makedirs(UPLOAD_DIR_CLOTHES_3D, exist_ok=True)
...
def create_clothes_3d_url(request: Request, clo_3d_path: Optional[str]) -> Optional[str]:
    if clo_3d_path:
        return f"{request.base_url}files/clothes/3d/{os.path.basename(clo_3d_path)}"
    return None
...
async def get_clothes(request: Request, repository: ClothesRepository = Depends()):
    ...
    for clothe in clothes:
        ...
        clothes_schema.clo_3d_url = create_clothes_3d_url(request, clothe.clo_3d) if clothe.clo_3d else None
...
async def get_clothes_item(clo_idx: int, request: Request, repository: ClothesRepository = Depends()):
    ...
    clothes_schema.clo_3d_url = create_clothes_3d_url(request, clothes_item.clo_3d) if clothes_item.clo_3d else None
...
async def create_clothes(
    ...
    clo_desc: str = Form(...),
    clo_price: int = Form(...),
    clo_3d: Optional[UploadFile] = File(None),
    ...
    # clo_3d 파일 처리
    if clo_3d:
        validate_3d_file(clo_3d)  # 3D 파일 유효성 검사 함수 호출
        unique_3d_filename = generate_unique_filename(clo_3d.filename)
        clo_3d_path = os.path.join(UPLOAD_DIR_CLOTHES_3D, unique_3d_filename)
        await save_upload_file(clo_3d, clo_3d_path)
    else:
        clo_3d_path = None
    ...
    clothes_data = {
        ...
        "clo_desc": clo_desc,
        "clo_price": clo_price,
        "clo_3d": clo_3d_path,  # 3D 파일 경로 저장,
        "clo_img1": file_path,
    ...
    clothes_schema.clo_3d_url = create_clothes_3d_url(request, clo_3d_path) if clo_3d_path else None
    return clothes_schema
...
async def update_clothes(
    ...
    clo_3d: Optional[UploadFile] = File(None),  # 변경: UploadFile로 수정
    ...
    # clo_3d 업데이트 처리
    if clo_3d:
        validate_3d_file(clo_3d)
        unique_3d_filename = generate_unique_filename(clo_3d.filename)
        clo_3d_path = os.path.join(UPLOAD_DIR_CLOTHES_3D, unique_3d_filename)
        await save_upload_file(clo_3d, clo_3d_path)
        if clothes_item.clo_3d:
            delete_file(clothes_item.clo_3d)
        clothes_item.clo_3d = clo_3d_path
    ...
    clothes_schema.clo_3d_url = create_clothes_3d_url(request, updated_clothes.clo_3d) if updated_clothes.clo_3d else None
    return clothes_schema
