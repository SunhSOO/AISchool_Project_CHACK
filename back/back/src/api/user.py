...
from fastapi import APIRouter, Depends, HTTPException, status , Request  # Request 추가
...
@router.post("/sign-up", response_model=UserSchema,summary="회원가입")
async def sign_up(
    ...
    user = repository.save_user(repository.model.create(
        ...
        user_gender=signup_request.user_gender  # 성별 추가
    ))
    return UserSchema.model_validate(user)

@router.post("/log-in", response_model=Token, summary="로그인")
async def log_in(
...
async def read_users_me(current_user: User = Depends(get_current_user)):  # 수정: async 및 Request 추가
