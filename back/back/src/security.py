...
if not ALGORITHM:
    raise ValueError("ALGORITHM 환경 변수가 설정되지 않았습니다.")

# 디버깅을 위한 출력 (개발 환경에서만 사용)
print(f"Loaded SECRET_KEY: {SECRET_KEY}")  # 추가된 코드
print(f"Using ALGORITHM: {ALGORITHM}")      # 추가된 코드
...
def create_access_token(data: dict, expires_delta: Optional[timedelta] = None):
    ...
    encoded_jwt = jwt.encode(to_encode, SECRET_KEY, algorithm=ALGORITHM)
     ...
     # 디버깅을 위한 출력 (개발 환경에서만 사용)
    print(f"Token data: {to_encode}")        # 추가된 코드
    print(f"Encoded JWT: {encoded_jwt}")     # 추가된 코드
