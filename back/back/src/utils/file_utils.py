...
#clo_3d
def validate_3d_file(upload_file: UploadFile):
    allowed_extensions = ['.obj', '.fbx', '.dae', '.3ds']  # 허용할 3D 모델 파일 확장자 목록
    extension = os.path.splitext(upload_file.filename)[1].lower()
    if extension not in allowed_extensions:
        raise HTTPException(status_code=400, detail="유효하지 않은 3D 모델 파일입니다.")
