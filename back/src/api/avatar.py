...
def create_avatar_urls(request: Request, avatar: Avatar) -> dict:
    base_url = str(request.base_url)
    return {
        "img_url": f"{base_url}files/avatars/images/{os.path.basename(avatar.img_path)}",
        "avatar_url": f"{base_url}files/avatars/models/{os.path.basename(avatar.avatar_path)}" if avatar.avatar_path else None
    }
...
async def get_avatars(request: Request, current_user: User = Depends(get_current_user), repository: AvatarRepository = Depends()):
    ...
    return [
        AvatarSchema.model_validate(avatar).model_copy(update=create_avatar_urls(request, avatar)) 
...
async def create_avatar(
    ...
    avatar_schema.img_url = f"{request.base_url}files/avatars/images/{unique_filename}"  # 수정: URL 설정
    avatar_schema.avatar_url = f"{request.base_url}files/avatars/models/{avatar_filename}"  # 수정: URL 설정
...
async def update_avatar(
    ...
    if img_file:
        ...
        avatar_schema.img_url = f"{request.base_url}files/avatars/images/{os.path.basename(updated_avatar.img_path)}"  # 수정: URL 설정
        avatar_schema.avatar_url = f"{request.base_url}files/avatars/models/{os.path.basename(updated_avatar.avatar_path)}" if updated_avatar.avatar_path else None
    return avatar_schema
