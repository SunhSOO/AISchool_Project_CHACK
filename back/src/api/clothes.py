...
UPLOAD_DIR = "uploads/clothes"  # 이미지 파일을 저장할 디렉토리

# 디렉토리가 존재하지 않으면 생성
os.makedirs(UPLOAD_DIR, exist_ok=True)

logger = logging.getLogger(__name__)
logging.basicConfig(level=logging.INFO)  # 로깅 설정 (필요에 따라 조정)
...
def create_clothes(
    ...
    # 고유한 파일명 생성
    unique_filename = f"{uuid.uuid4()}_{clo_img1.filename}"
    file_path = os.path.join(UPLOAD_DIR, unique_filename)
    
    # 파일 저장
    try:
        with open(file_path, "wb") as buffer:
            shutil.copyfileobj(clo_img1.file, buffer)
